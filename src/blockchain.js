...
class Blockchain {
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
           const block = self.chain.find(blk => blk.hash === hash);
            ...
            try {
                self.chain.forEach(async (block) => {
                    if(await block.validate()) {
                        ...
                        if(prevBlock && block.previousBlockHash && prevBlock.hash !== block.previousBlockHash) {
                            errorLog.push({hash: block.hash, message: 'previousBlockHash is invalid'});
                        }
                    }
                })

                resolve({errorLog, isChainValid: errorLog.length === 0});
            } catch (error) {
                reject(error.message)
            }
        });
